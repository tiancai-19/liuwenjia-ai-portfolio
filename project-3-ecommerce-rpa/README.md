...
- **模板库导出 / 导入**（④）：收藏成果可一键导出为 xlsx 交付物（每行含商品名/类目/3 标题/3 卖点/详情/主图提示词），直接给运营复制上新，或作为影刀 RPA 回写后台的输入文件；编辑过的表格可再导入回模板库，实现备份 / 换机迁移 / 批量改文案
...
- 使用（导出/导入）：④ 模板库 →「导出为 Excel（交付物）」拿到成果表 → 本地可编辑 →「导入 Excel」批量更新回模板库
